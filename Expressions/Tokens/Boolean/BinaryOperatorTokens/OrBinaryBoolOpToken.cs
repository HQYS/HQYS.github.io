...
{
    internal class XorBinaryBoolOpToken : IBinaryBoolOpToken
    {
        public static string Ident => "XOR";

        string IBinaryOperatorToken.Ident => Ident;

        TokenTypeEnum IToken.Type => TokenTypeEnum.BinaryOperator;

        BinaryBoolOpTypeEnum IBinaryBoolOpToken.OperationType => BinaryBoolOpTypeEnum.Xor;
        ...
        IConstantToken IBinaryOperatorToken.Calc(IConstantToken a, IConstantToken b)
        {
            ...
            {
                throw new Exception("Для операции логического исключающего 'ИЛИ'ожидались параметры типа 'boolean'.");
            }
